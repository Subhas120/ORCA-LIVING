...
from backend.services.m2_adapter import M2Adapter, M2AdapterError, InsufficientEvidenceError
...
def _run_decision(request: DecisionRequest) -> DecisionResponse:
    ...
    """
    try:
        world_state, proposals, safety_evals = m2_adapter.get_marine_state_and_safety(request)
    ...
    try:
        decision_intel = m1_service.run_decision_pipeline(
            world_state, proposals, safety_evals, request
        )
    ...
    return ResponseAdapter.adapt(decision_intel, status)
...
async def voice_endpoint(
    ...
    try:
        ...
        if audio is not None:
            ...
            result = pipeline.process_audio_bytes(audio_bytes, _run_decision)
        elif text is not None:
            result = pipeline.process_text(text, _run_decision)
        else:
            raise HTTPException(
                status_code=400,
...
async def voice_text_endpoint(request: VoiceTextRequest):
    """Text-only voice endpoint (JSON body, no file upload).

    Convenience endpoint for testing and non-audio clients.
    Accepts JSON with a text field and returns voice pipeline results.
    """
    try:
        pipeline = VoicePipeline()
        result = pipeline.process_text(request.text, _run_decision)
        return VoiceAPIResponse(**result.to_dict())
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Voice pipeline error: {e}")
